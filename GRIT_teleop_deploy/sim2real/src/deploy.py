...
class Controller:
    def __init__(self, args, ctrl_cfg):
        ...
        self.startup_interpolation_s = float(
            getattr(self.config, "startup_interpolation_s", 2.0)
        )
        if self.startup_interpolation_s <= 0.0:
            raise ValueError("startup_interpolation_s must be positive")
        self.startup_kp_floor = float(
            getattr(self.config, "startup_kp_floor", 0.10)
        )
        if not 0.0 < self.startup_kp_floor <= 1.0:
            raise ValueError("startup_kp_floor must be in (0, 1]")
        ...
        # The sim2sim gantry release is represented by an A edge.  If NPZ
        # playback was already queued by --auto-start, ignore that synthetic
        # edge so it cannot restart the same motion.
        self._ignore_next_npz_a_edge = False
        self.sticks = {
            "lx": 0.0,
            "ly": 0.0,
            "rx": 0.0,
        ...
        # Hardware bridge advertises this flag only after native motion-service
        # release. Missing field means a simulator/legacy bridge is ready.
        self.bridge_control_active = False
    ...
    def _consume_low_state(self, msg) -> bool:
        ...
        # The hardware bridge reports False until its native motion-service
        # handoff is complete. Sim/legacy bridges omit this field and are
        # treated as ready for backward compatibility.
        self.bridge_control_active = bool(msg.get("bridge_control_active", True))
        ...
        tracking_state_id = msg.get("tracking_state_id", None)
        self.tracking_state_id = None if tracking_state_id is None else int(tracking_state_id)
    ...
    def send_cmd(self):
        self.transport.send_command(
            q_des=self.cmd_q,
            # Reference evaluation adds FK inputs to the UDP command.  The
            # simulator does not consume qd_des, so omit it in that mode to
            # keep the datagram below a 1500-byte network MTU.
            qd_des=None if bool(getattr(self.args, "publish_reference", False)) else self.cmd_qd,
            kp=self.cmd_kp,
            kd=self.cmd_kd,
    ...
    def _publish_reference_debug_state(self):
        ...
        phases = getattr(policy, "ref_evaluation_phase", None)
        motion_frames = getattr(policy, "ref_motion_frame", None)
        phase = "other" if phases is None else str(phases[idx])
        motion_frame = -1 if motion_frames is None else int(motion_frames[idx])
        motion_frame_count = 0 if motion_frames is None else int(np.count_nonzero(motion_frames >= 0))
        self.extra_command["reference"] = {
            ...
            "tracking_state_id": getattr(self, "tracking_state_id", None),
            "state_sequence": self.last_state_seq,
        }
    ...
    def _apply_gamepad_npz_request(self) -> None:
        """Start local NPZ playback on a rising edge of the G1 gamepad A button."""
        if (
            getattr(self, "_ignore_next_npz_a_edge", False)
            and self.btn_rise.get("A", False)
        ):
            self._ignore_next_npz_a_edge = False
            print("[Deploy] Ignoring sim2sim gantry-release A edge after auto-start.")
            return
        if not self.btn_rise.get("A", False):
            return
        current_policy = self.current_policy
        if current_policy is None or getattr(current_policy, "motion_source", None) != "npz":
            return
        play_from_start = getattr(current_policy.source, "play_from_start", None)
        if callable(play_from_start):
            print("[Deploy] G1 gamepad A received; starting slow first-frame transition.")
            play_from_start()
    ...
    def zero_torque_state(self):
        ...
        if bool(getattr(self.args, "auto_start", False)):
            ...
            return
        print(
            "Waiting for START on the G1 gamepad "
            "(or keyboard 's'; SELECT is the latched emergency stop)..."
        )
        while not self.buttons["start"] and not self._keyboard_start_event.is_set():
            ...
            if self.buttons["stop"]:
                raise DampingRequested("G1 gamepad SELECT emergency stop")
            self.set_zero_cmd()
            self.send_cmd()

    def move_to_default_qpos(self):
        print(
            "Moving to default pose with minimum-jerk interpolation "
            f"({self.startup_interpolation_s:.2f}s)..."
        )
        num_step = max(1, int(round(self.startup_interpolation_s / self.control_dt)))

        # The hardware bridge releases native motion control only after START.
        # Wait for its explicit handoff acknowledgement before sending active
        # PD commands, so a slow ReleaseMode RPC cannot discard early frames.
        while not getattr(self, "bridge_control_active", True):
            self.process_state(wait_next=True)
            if self.buttons["stop"]:
                raise DampingRequested("G1 gamepad SELECT emergency stop")
            self.set_zero_cmd()
        ...
        # Refresh the measured pose immediately after START. The robot may
        # have moved slightly while it was held in damping before START.
        self.process_state(wait_next=True)
        if self.buttons["stop"]:
            raise DampingRequested("G1 gamepad SELECT emergency stop")
        ...
        for t in range(1, num_step + 1):
            ...
            # Do not step from damping (Kp=0, Kd=8) to full leg gains on the
            # first frame. Start with a small holding gain and ramp both gains
            # with the same smooth curve as the position target.
            gain = self.startup_kp_floor + (1.0 - self.startup_kp_floor) * alpha
            self.cmd_q[:] = init_dof_pos * (1 - alpha) + self.default_qpos * alpha
            self.cmd_qd[:] = 0.0
            self.cmd_kp[:] = self.kps * gain
            self.cmd_kd[:] = 8.0 * (1.0 - gain) + self.kds * gain
            self.send_cmd()
    ...
    def default_qpos_state(self):
        ...
        if npz_control:
            ...
            print("NPZ tracking policy active in default pose; press gamepad A to play.")
        elif auto_start:
            initial_policy = tracking_policy
        ...
        if npz_control and (auto_start or self._keyboard_play_event.is_set()):
            ...
            if callable(play_from_start):
                play_from_start()
            if auto_start:
                self._ignore_next_npz_a_edge = True
        self.cmd_enable = 1
        self.send_cmd()
    ...
    def run(self):
        ...
        while True:
            ...
            self._apply_gamepad_npz_request()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Damping requested (gamepad SELECT or keyboard 'x'). Entering damping mode...")
