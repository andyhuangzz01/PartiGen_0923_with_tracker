...
class ReferenceTrackingPolicy(Policy):
    def __init__(self, name: str, policy_cfg: DictToClass, controller):
        ...
        # Evaluation-only labels stay frame-aligned with the reference arrays.
        # They let sim2sim distinguish the generated clip from its transition
        # and return-to-default frames without inferring boundaries from time.
        self.ref_evaluation_phase: Optional[np.ndarray] = None
        self.ref_motion_frame: Optional[np.ndarray] = None
    ...
    def deactivate(self):
        ...
        self.ref_evaluation_phase = None
        self.ref_motion_frame = None
        super().deactivate()
    ...
    def append_ref_frames(self, frames: Dict[str, np.ndarray]) -> None:
        ...
        phase = np.asarray(
            frames.get("evaluation_phase", np.full(j.shape[0], "other")),
            dtype="<U16",
        ).reshape(-1)
        motion_frame = np.asarray(
            frames.get("motion_frame", np.full(j.shape[0], -1)),
            dtype=np.int64,
        ).reshape(-1)
        if phase.shape[0] != j.shape[0] or motion_frame.shape[0] != j.shape[0]:
            raise ValueError("evaluation metadata must have one entry per reference frame")
        ...
        if self.ref_joint_pos is None or self.ref_root_quat is None or self.ref_root_pos is None or self.ref_len <= 0:
            ...
            self.ref_evaluation_phase = phase.copy()
            self.ref_motion_frame = motion_frame.copy()
        else:
            if getattr(self, "ref_evaluation_phase", None) is None:
                self.ref_evaluation_phase = np.full(self.ref_len, "other", dtype="<U16")
            if getattr(self, "ref_motion_frame", None) is None:
                self.ref_motion_frame = np.full(self.ref_len, -1, dtype=np.int64)
            ...
            self.ref_evaluation_phase = np.concatenate([self.ref_evaluation_phase, phase], axis=0)
            self.ref_motion_frame = np.concatenate([self.ref_motion_frame, motion_frame], axis=0)
    ...
    def discard_future_ref_frames(self) -> int:
        ...
        if getattr(self, "ref_evaluation_phase", None) is not None:
            self.ref_evaluation_phase = self.ref_evaluation_phase[:keep]
        if getattr(self, "ref_motion_frame", None) is not None:
            self.ref_motion_frame = self.ref_motion_frame[:keep]
        self.ref_len = keep
        self.current_done = True
        return discarded
    ...
    def _trim_ref_prefix(self) -> None:
        ...
        if getattr(self, "ref_evaluation_phase", None) is not None:
            self.ref_evaluation_phase = self.ref_evaluation_phase[drop:]
        if getattr(self, "ref_motion_frame", None) is not None:
            self.ref_motion_frame = self.ref_motion_frame[drop:]
        self.ref_idx -= drop
        self.ref_len = int(self.ref_joint_pos.shape[0])
        self.current_done = (self.ref_idx >= self.ref_len - 1)
