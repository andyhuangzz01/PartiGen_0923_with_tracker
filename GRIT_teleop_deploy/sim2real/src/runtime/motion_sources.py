...
class MotionSourceBase(ABC):
    ...
    def _build_transition_prefix(
        ...
        *,
        ...
        t_steps = int(
            self.policy.transition_steps
            if transition_steps is None
            else transition_steps
        )
        if t_steps <= 0:
            return self._empty_frames(self.policy.n_joints)

        joints_tr = _linspace_rows(
            anchor["joint_pos"], tgt_first["joint_pos"], t_steps,
            easing=transition_easing,
        )
        root_pos_tr = _linspace_rows(
            anchor["root_pos"], tgt_first["root_pos"], t_steps,
            easing=transition_easing,
        )
        root_quat_tr = _slerp(
            anchor["root_quat"], tgt_first["root_quat"], t_steps,
            easing=transition_easing,
        )
    ...
    def append_motion_from_tail(
        ...
        *,
        ...
        trans_motion = self._build_transition_prefix(
            anchor,
            tgt_first,
        ...
        is_primary = name == getattr(self, "primary", None)
        n_prefix = int(trans_motion["joint_pos"].shape[0])
        n_motion = int(aligned_motion["joint_pos"].shape[0])
        segment["evaluation_phase"] = np.concatenate([
            np.full(n_prefix, "transition_in" if is_primary else "other"),
            np.full(n_motion, "motion" if is_primary else "other"),
        ])
        segment["motion_frame"] = np.concatenate([
            np.full(n_prefix, -1, dtype=np.int64),
            np.arange(n_motion, dtype=np.int64) if is_primary else np.full(n_motion, -1, dtype=np.int64),
        ])
        ...
        print(
            ...
            f"ref_len={self.policy.ref_len}, transition={trans_motion['joint_pos'].shape[0]}, "
            f"easing={transition_easing}"
        )
        return True
...
class LocalNpzMotionSource(MotionSourceBase):
    ...
    def __init__(self, policy: "ReferenceTrackingPolicy", policy_cfg: DictToClass):
        ...
        # MotionSourceBase assigns self.policy; use the constructor argument
        # here because these settings are resolved before super().__init__().
        default_transition_s = float(policy.transition_steps) / reference_fps
        self.first_frame_transition_s = float(
            npz_cfg.get("first_frame_transition_s", default_transition_s)
        )
        if self.first_frame_transition_s < 0.0:
            raise ValueError("motion_source.npz.first_frame_transition_s must be non-negative")
        self.first_frame_transition_steps = int(
            round(self.first_frame_transition_s * reference_fps)
        )
        ...
        print(
            ...
            f"first_frame_transition={self.first_frame_transition_steps} steps/"
            f"{self.first_frame_transition_s:.2f}s"
        )
    ...
    def play_from_start(self) -> bool:
        ...
        transition_steps = int(
            getattr(
                self,
                "first_frame_transition_steps",
                getattr(self.policy, "transition_steps", 100),
            )
        )
        appended = self.append_motion_from_tail(
            self.primary,
            transition_steps=transition_steps,
