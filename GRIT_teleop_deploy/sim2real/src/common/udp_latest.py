...
class UDPLatestReceiver:
    def __init__(
        ...
        *,
        ...
        try:
            # A UDP control/state port must belong to exactly one bridge or
            # controller.  SO_REUSEADDR lets multiple processes bind the same
            # port on Linux, which silently mixes robot states and commands.
            self._sock.bind((host, port))
        except OSError as exc:
            self._sock.close()
            raise OSError(
                exc.errno,
                f"Cannot bind UDP {host}:{port}; another GRIT process may "
                "already be running",
            ) from exc
