...
def _command_payload(
    *,
    ...
    if qd_des is not None:
        payload["qd_des"] = np.asarray(qd_des, dtype=np.float32)
...
def _state_payload(
    *,
    ...
    state_receive_time_ns: int | None = None,
    tracking_state_id: int | None = None,
) -> dict[str, Any]:
    payload = {
    ...
    if tracking_state_id is not None:
        payload["tracking_state_id"] = int(tracking_state_id)
    return payload
