...
struct FrequencyConfig {
  ...
  // Unitree G1 low-level examples publish rt/lowcmd at 500 Hz.  Python policy
  // updates remain at 50 Hz; the bridge republishes the latest command here.
  double command_publish_hz = 500.0;
...
BridgeConfig load_config(const std::string & path)
{
  ...
  cfg.freq.command_publish_hz = yaml_value_or<double>(freq["command_publish_hz"], 500.0);
  ...
  if (cfg.freq.command_publish_hz <= 0.0) {
    throw std::runtime_error("freq.command_publish_hz must be positive");
  }
...
class UdpLatestSender {
 public:
  ...
  void send_state(
      ...
      const std::vector<float> & motor_temp, bool control_active)
  {
    ...
    meta << "{\"q\":" << ndarray_meta(q_ref) << ",\"dq\":" << ndarray_meta(dq_ref)
         ...
         << ",\"bridge_control_active\":" << bool_text(control_active)
         << ",\"buttons\":{"
 ...
 public:
        ...
        state_sender_(cfg_.udp.state_host, cfg_.udp.state_port, cfg_.udp.sndbuf_bytes)
  {
    ...
    std::cout << "[G1Bridge] freq: physical_hz=" << cfg_.freq.physical_hz
              << " command_publish_hz=" << cfg_.freq.command_publish_hz
              ...
              << std::endl;
    std::cout << "[G1Bridge] command mode: UDP latest-command cache -> physical-rate DDS writer, mode_pr="
  ...
  void activate_low_level()
  {
    ...
    wait_for_gamepad_start();
    if (gamepad_estop_latched_.load(std::memory_order_acquire)) {
      throw std::runtime_error(
          "G1 gamepad SELECT emergency stop is latched; release SELECT and restart the bridge");
    }
    ...
    start_command_writer_thread();
    start_command_watchdog_thread();
    if (gamepad_estop_latched_.load(std::memory_order_acquire)) {
      std::cerr << "[G1Bridge] Low-level handoff completed in latched damping mode; "
                   "restart the bridge to clear SELECT emergency stop"
                << std::endl;
    }
 ...
 private:
  void wait_for_initial_command()
  {
    ...
    while (!have_initial_valid_command_.load(std::memory_order_acquire)) {
      if (gamepad_estop_latched_.load(std::memory_order_acquire)) {
        throw std::runtime_error(
            "G1 gamepad SELECT emergency stop is latched; restart the bridge to clear it");
      }
      if (g_stop_requested.load(std::memory_order_relaxed)) {
  ...
  void wait_for_gamepad_start()
  {
    std::cout << "[G1Bridge] Waiting for G1 gamepad START before releasing native motion service"
              << std::endl;
    while (!gamepad_start_requested_.load(std::memory_order_acquire)) {
      if (gamepad_estop_latched_.load(std::memory_order_acquire)) {
        throw std::runtime_error(
            "G1 gamepad SELECT emergency stop is latched; restart the bridge to clear it");
      }
      if (g_stop_requested.load(std::memory_order_relaxed)) {
        throw std::runtime_error("Interrupted while waiting for G1 gamepad START");
      }
      std::this_thread::sleep_for(std::chrono::milliseconds(20));
    }
    std::cout << "[G1Bridge] G1 gamepad START received; releasing native motion service" << std::endl;
  }
  ...
  {
    ScopedTerminalRawMode raw_mode(STDIN_FILENO);
    std::cout << "[G1Bridge] gamepad: START=stand, A=play, SELECT=latched damping E-stop; "
                 "stdin: s=start, a=A, x=stop, q=emergency exit"
              << (raw_mode.enabled() ? " (single-key tty mode)" : " (line-buffered/pipe mode)") << std::endl;
  ...
  void handle_stdin_button(char input)
  {
    ...
    if (ch == 's') {
      gamepad_start_requested_.store(true, std::memory_order_release);
  ...
  void send_state_snapshot(const StateSnapshot & snapshot)
  {
    ...
    const RemoteState gamepad = parse_remote(low_state.wireless_remote());
    if (gamepad.stop) {
      latch_gamepad_estop();
    }
    const RemoteState remote = apply_stdin_button_overrides(gamepad);
    if (remote.start) {
      gamepad_start_requested_.store(true, std::memory_order_release);
    }
    ...
    try {
      state_sender_.send_state(
          q_policy, dq_policy, quat, gyro, linacc, remote, motor_temp,
          low_level_active_.load(std::memory_order_acquire));
  ...
  void on_udp_command(const LatestPacket & packet)
  {
    if (fatal_shutdown_requested_.load(std::memory_order_relaxed) ||
        g_stop_requested.load(std::memory_order_relaxed) ||
        gamepad_estop_latched_.load(std::memory_order_acquire)) {
      return;
    }
    ...
    try {
      ...
      bool timeout_recovered = false;
      {
        ...
        {
          std::lock_guard<std::mutex> latest_lock(latest_command_mutex_);
          latest_command_ = cmd;
        }
  ...
  void latch_gamepad_estop()
  {
    if (!gamepad_estop_latched_.exchange(true, std::memory_order_acq_rel)) {
      gamepad_estop_event_count_.fetch_add(1, std::memory_order_relaxed);
      std::cerr << "[G1Bridge] G1 gamepad SELECT E-STOP LATCHED: rejecting policy commands "
                   "and continuously publishing damping. Restart bridge to clear."
                << std::endl;
    }
    if (low_level_active_.load(std::memory_order_acquire)) {
  ...
  std::chrono::nanoseconds command_publish_period() const
  {
    const auto period = std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::duration<double>(1.0 / cfg_.freq.command_publish_hz));
    if (period.count() <= 0) {
      throw std::runtime_error("Computed command publish period is not positive");
    }
    return period;
  }

  void start_command_writer_thread()
  {
    command_writer_stop_.store(false, std::memory_order_relaxed);
    command_writer_thread_ = std::thread([this]() { command_writer_loop(); });
  }

  void stop_command_writer_thread()
  {
    command_writer_stop_.store(true, std::memory_order_relaxed);
  ...
  {
    const int timer_fd = create_periodic_timer_fd(command_publish_period());
    try {
      while (!command_writer_stop_.load(std::memory_order_relaxed) &&
             !g_stop_requested.load(std::memory_order_relaxed)) {
        pollfd pfd{};
        pfd.fd = timer_fd;
        pfd.events = POLLIN;
        ...
        if (ready < 0) {
          ...
          throw std::runtime_error(errno_text("poll(command timer) failed"));
        }
        if (ready == 0 || (pfd.revents & POLLIN) == 0) {
          continue;
        }
        ...
        const uint64_t expirations = read_timer_expirations(timer_fd);
        if (expirations > 1) {
          command_writer_missed_period_count_.fetch_add(
              expirations - 1, std::memory_order_relaxed);
        }
        ...
        if (gamepad_estop_latched_.load(std::memory_order_acquire)) {
          write_damping_command_locked();
          lowcmd_publish_count_.fetch_add(1, std::memory_order_relaxed);
          continue;
        }

        std::optional<LowCmd> latest;
        {
          std::lock_guard<std::mutex> latest_lock(latest_command_mutex_);
          latest = latest_command_;
        }
        if (latest) {
          lowcmd_publisher_->Write(*latest);
        } else {
          // Keep the robot in damping until the first active policy command
          // arrives after the low-level handoff.
          write_damping_command_locked();
        }
        lowcmd_publish_count_.fetch_add(1, std::memory_order_relaxed);
      }
    } catch (const std::exception & exc) {
      command_writer_error_count_.fetch_add(1, std::memory_order_relaxed);
      std::cerr << "[G1Bridge] Command writer failed: " << exc.what() << std::endl;
      g_stop_requested.store(true, std::memory_order_relaxed);
    }
    ::close(timer_fd);
  }
  ...
  void log_rates()
  {
    ...
    const uint64_t lowcmd_publishes = lowcmd_publish_count_.exchange(0, std::memory_order_relaxed);
    const uint64_t command_writer_missed_periods =
        command_writer_missed_period_count_.exchange(0, std::memory_order_relaxed);
    const uint64_t command_writer_errors = command_writer_error_count_.exchange(0, std::memory_order_relaxed);
    ...
    std::cout << "[G1Bridge] rates over " << std::fixed << std::setprecision(2) << elapsed
              ...
              << " | lowcmd_publish=" << (static_cast<double>(lowcmd_publishes) / elapsed) << " Hz ("
              << lowcmd_publishes << ")"
              << " writer_missed_periods=" << command_writer_missed_periods
              << " writer_errors=" << command_writer_errors
  ...
  std::atomic<bool> command_writer_stop_{false};
  std::atomic<bool> low_level_active_{false};
  std::atomic<bool> command_timeout_active_{false};
  std::atomic<bool> gamepad_estop_latched_{false};
  std::atomic<bool> gamepad_start_requested_{false};
  ...
  std::atomic<uint64_t> command_writer_missed_period_count_{0};
  std::atomic<uint64_t> command_writer_error_count_{0};
  std::atomic<uint64_t> gamepad_estop_event_count_{0};
  std::atomic<int64_t> latest_cmd_seq_{-1};
  std::atomic<bool> have_latest_cmd_seq_{false};

  std::mutex policy_delay_mutex_;
  std::mutex latest_command_mutex_;
  std::optional<LowCmd> latest_command_;
