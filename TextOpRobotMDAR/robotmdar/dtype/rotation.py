...
def quat_rotate(q: Tensor, v: Tensor, w_last: bool) -> Tensor:
    ...
    c = (q_vec * torch.bmm(q_vec.view(shape[0], 1, 3), v.view(
...
@torch.jit.script
def quat_to_angle_axis_v2(x: Tensor, w_last: bool) -> Tuple[Tensor, Tensor]:
    """
    ...
    if w_last:
        ...
        axis = x[..., 1:]
    s = 2 * (w**2) - 1
    angle = s.clamp(-1 + 1e-6, 1 - 1e-6).arccos()  # just to be safe
    axis_normed = axis / axis.norm(p=2, dim=-1, keepdim=True).clamp(min=1e-9)
    return angle, axis_normed
...
@torch.jit.script
def transform_from_rotation_translation(r: Optional[torch.Tensor] = None,
...
def transform_mul(x, y):
    ...
    z = transform_from_rotation_translation(
        r=quat_mul_norm(transform_rotation(x),
...
@torch.jit.script
def euler_angles_to_quaternion_v2(rpy: torch.Tensor):
...
def quaternion_to_euler_angles(q: torch.Tensor) -> torch.Tensor:
    """
    Convert quaternion (xyzw) to Euler angles (roll, pitch, yaw) using XYZ convention.
    Input: q (..., 4) with xyzw format
    Output: (..., 3) → roll, pitch, yaw
    """
    ...
    # Pitch (y-axis)
    sinp = 2 * (w * y - z * x)
    pitch = torch.where(sinp.abs() >= 1,
                        torch.sign(sinp) * (torch.pi / 2), torch.asin(sinp))
...
def euler_angles_to_quaternion(euler: torch.Tensor) -> torch.Tensor:
    """
    Convert Euler angles to quaternion.
    Input: euler (..., 3) → roll, pitch, yaw
    Output: quaternion (..., 4) in xyzw order
    """
